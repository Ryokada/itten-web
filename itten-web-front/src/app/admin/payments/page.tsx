...
import { useForm, SubmitHandler, useWatch, Controller, set } from 'react-hook-form';
...
const PaymentForm = () => {
    ...
    const onSubmit: SubmitHandler<PaymentInput> = async (data) => {
        ...
        try {
            ...
            const addPayment = httpsCallable<PaymentInput, PaymentResult>(
                functions,
                'accounting-addPayment',
            );
            const response = await addPayment(updateData);
            setResult(response.data);
            console.log('精算を追加しました。', response.data);
    ...
    useEffect(() => {
        if (me) {
            setValue('paidMenberName', me.name);
        }
    }, [me, setValue]);
        ...
        <div>
            <form onSubmit={handleSubmit(onSubmit)} className='p-4 space-y-4 bg-gray-100'>
                ...
                <div className='flex flex-col'>
                    ...
                    <textarea
                        id='description'
                        {...register('description', { required: '必須です' })}
                        className='p-2 border rounded-md'
                    />
                </div>
                <div className='flex flex-col'>
                    ...
                    {errors.participationFeeIncome && (
                        <p className='text-red-500 text-xs'>
                            {errors.participationFeeIncome.message}
                        </p>
                    )}
                </div>
            ...
            <Dialog isOpen={isOpenDoneDialog} onClose={() => setIsOpenDoneDialog(false)}>
                <div>
                    <h1 className='mb-2 text-lg'>精算データを追加しました。</h1>
                    <a
                    ...
                    <div className='flex w-full mt-5 space-x-3'>
                        <button
                            onClick={() => setIsOpenDoneDialog(false)}
                            className='w-full p-2 bg-gray-400 text-white rounded-md hover:bg-gray-600'
                        >
                            閉じる
                        </button>
                    </div>
